...
def cal_maf(line,wdmaf,lvmaf):
    ...
    if (cal_wd <= wdmaf or cal_wd >= 1-wdmaf) and 1-lvmaf > cal_lv > lvmaf:
        return line
    # wild maf = 0.01 (KEEP) AND landrace variety maf = 0.05 (KEEP)
    #if  1-wdmaf >= cal_wd >= wdmaf and 1-lvmaf > cal_lv > lvmaf:
    #   return line
...
def wheat_maf_filter(ifile,wdmaf,lvmaf,wfile,chr):
        ...
        for line in f_in:
            ...
            if line[0][0:2] == '##':
                ...
                if line[0] == chr:
                    ...
                    if new_line != None:
                        aa = new_line[0:2] + new_line[9:44] + new_line[45:53] + new_line[54::]
                        matrix = trans_GT(aa)
                        wfile.write('{}\n'.format('\t'.join(matrix)))
                    else:
                        continue
...
wheat_maf_filter(ifile=args.input,wdmaf=args.wdmaf,lvmaf=args.lvmaf,wfile=args.output,chr=args.chr)
